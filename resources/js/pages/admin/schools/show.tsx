...
export default function SchoolShow() {
    ...
    const supervisor = school.admin.user;
    ...
    return (
        <AdminLayout
            ...
            description={`Detailed view of ${school.name} and its supervisor.`}
            header={
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <Card>
                        ...
                        <CardContent>
                             {supervisor.documents && supervisor.documents.length > 0 ? (
                                <ul className="space-y-4">
                                    {supervisor.documents.map((doc) => (
                ...
                <div className="space-y-8">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-2">
                            <DetailItem label="Full Name" value={supervisor.name} />
                            <DetailItem label="Email Address">
                                <a href={`mailto:${supervisor.email}`} className="text-primary hover:underline flex items-center gap-2">
                                    <Mail className="w-4 h-4" /> {supervisor.email}
                                </a>
                            </DetailItem>
                             <DetailItem label="Phone Number">
                                <a href={`tel:${supervisor.phone_zone}${supervisor.phone}`} className="text-primary hover:underline flex items-center gap-2">
                                    <Phone className="w-4 h-4" /> {supervisor.phone_zone}{supervisor.phone}
                                </a>
                            </DetailItem>
                            <DetailItem label="WhatsApp">
                                <a href={`https://wa.me/${supervisor.whatsapp_zone}${supervisor.whatsapp}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-2">
                                   <Phone className="w-4 h-4" /> {supervisor.whatsapp_zone}{supervisor.whatsapp}
                                </a>
                            </DetailItem>
                            <DetailItem label="Gender" value={supervisor.gender} />
                            <DetailItem label="Date of Birth" value={supervisor.birth_date} />
                            <DetailItem label="Country" value={supervisor.country} />
                            <DetailItem label="City" value={supervisor.city} />
                            <DetailItem label="Residence" value={supervisor.residence} />
                        </CardContent>
                    </Card>
